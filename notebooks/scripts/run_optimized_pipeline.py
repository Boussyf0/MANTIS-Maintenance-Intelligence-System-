...
try:
    ...
    plt.plot(
        fpr_def,
        tpr_def,
        label=f'Default Isolation Forest (AUC = {roc_auc_score(train_feat["label"], y_scores_iso_default):.2f})',
        linestyle="--",
    )
    ...
    plt.plot(
        fpr_opt,
        tpr_opt,
        label=f'Optimized Isolation Forest (AUC = {roc_auc_score(train_feat["label"], y_scores_iso):.2f})',
        linewidth=2,
    )
    ...
    plt.plot(
        fpr_rf,
        tpr_rf,
        label=f"Random Forest (Supervised) (AUC = {roc_auc_score(y_val, y_probs_rf):.2f})",
    )
